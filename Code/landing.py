...
foot_points = DM([1, 0.6, 0,
                     1,-0.6, 0,
                    -1, 0.6, 0,
                    -1,-0.6, 0])
initial_body_position  = vertcat(0,0,.8*sqrt(2),0,0,0,0,0,0,0,0,0)
leg_positions  = quad.inverse_kinematics(initial_body_position, foot_points)
...
terminal_body_position = vertcat(0,0,.8*sqrt(2),0,0,0,1,0,2,0,0,0)
...
f = -10*(vx_list[-1]**2 + vz_list[-1]**2) # take-off velocity objective
for i in range(N-1):
    u_i   = get_control(i)
    ...
    f +=  0.5 * dt * (0.01*(u_i.T @ u_i + u_ip1.T @ u_ip1) + 0*(tau.T @ tau + tau_ip1.T @ tau_ip1))
################### End of objective ###################
...
lbg = [0.]*(G.shape[0]-8*N) + [-inf]*8*N
ubg = [0.]*(G.shape[0]-8*N) + [ 0  ]*8*N
#################### End of bounds setup ###################
...
for i in range(12*N):
    x0 += [0]
x0 += [5] # initial guess for total time T
